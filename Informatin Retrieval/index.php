...
ob_start();
?>
<div id="image" align="center"><img src="img/crawl.png" /> </div>
...
if (count($words))
{
    ...
    $query = 'SELECT DISTINCT D.DOCUMENT_URL, D.DOCUMENT_TITLE, ' .
        'D.DESCRIPTION FROM CPT_SEARCH_DOCUMENT D ';
    foreach ($words as $index => $word)
    {
        $join .= sprintf(
            'JOIN CPT_SEARCH_INDEX I%d ON D.DOCUMENT_ID = I%d.DOCUMENT_ID ' .
            'JOIN CPT_SEARCH_TERM T%d ON I%d.TERM_ID = T%d.TERM_ID ',
            $index, $index, $index, $index, $index);
    
        $where .= sprintf('T%d.TERM_VALUE LIKE "%s" AND ', $index,
